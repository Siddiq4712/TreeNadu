...
const buildEventPublicId = (pinCode) => {
  const first = randomInt(10000000, 100000000).toString();
  const second = randomInt(10000000, 100000000).toString();
  return `${pinCode}-${first}${second}`;
};

export const generateUniqueEventId = async (pinCode) => {
  ...
  for (let attempt = 0; attempt < 25; attempt += 1) {
    const eventId = buildEventPublicId(normalizedPinCode);
    const existingEvent = await db.Event.exists({ event_id: eventId });
  ...
  throw { status: 500, message: 'Unable to generate a unique event ID. Please try again.' };
};
...
export const createEvent = async (eventData, creatorId) => {
  ...
  const existingEvent = await db.Event.exists({ event_id: eventId });
  if (existingEvent) {
    throw { status: 409, message: 'Event ID already exists. Please generate a new event ID.' };
  }
