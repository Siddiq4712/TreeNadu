...
const CreateEvent = () => {
  ...
  const [eventIdLoading, setEventIdLoading] = useState(false);
  ...
  useEffect(() => {
    const pinCode = form.pin_code.trim();
    ...
    let cancelled = false;
    setEventIdLoading(true);

    api.get('/events/generate-id', { params: { pin_code: pinCode } })
      .then((res) => {
        if (cancelled) {
          return;
        }
      ...
      .catch((err) => {
        ...
        console.error('CreateEvent generate event id failed:', err);
        setForm((current) => ({
          ...current,
          location_code: pinCode,
          event_id: '',
        }));
        showToast('Unable to generate a unique event ID. Please try again.', 'error');
      })
      .finally(() => {
        if (!cancelled) {
          setEventIdLoading(false);
  ...
  }, [form.pin_code, isEditMode, showToast]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (eventIdLoading || !form.event_id) {
      const message = 'Please wait until a unique Event ID is generated.';
      setError(message);
      showToast(message, 'error');
      setLoading(false);
      return;
    ...
    try {
      ...
      if (!isEditMode && err.response?.status === 409) {
        try {
          const res = await api.get('/events/generate-id', { params: { pin_code: trimmedPinCode } });
          updateField('event_id', res.data.event_id || '');
        } catch (regenerateError) {
          console.error('CreateEvent regenerate event id failed:', regenerateError);
        }
      }
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '0.85fr 1.15fr', gap: '24px' }}>
        ...
        >
          ...
          <div style={{ display: 'grid', gap: '14px', marginTop: '26px' }}>
            {[
              ['Auto Event ID', eventIdLoading ? 'Generating...' : form.event_id || 'Enter PIN code'],
              ['Event Title', form.title || 'Name this event'],
        ...
        >
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ display: 'block', fontWeight: 700, marginBottom: '8px', color: '#163126' }}>Event Title</label>
              <input
                value={form.title}
            ...
            <button
              type="submit"
              disabled={loading || eventLoading || eventIdLoading}
              style={{
                width: '100%',
                border: 'none',
            ...
            >
              {loading ? (isEditMode ? 'Saving Event...' : 'Creating Event...') : eventIdLoading ? 'Generating Event ID...' : (isEditMode ? 'Save Event Changes' : 'Create Event')}
            </button>
          </form>
            </>
          )}
